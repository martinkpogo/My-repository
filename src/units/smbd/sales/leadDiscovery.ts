...
export async function handleLeadDiscoverySignal(env: Env, chatId: number, threadId: number | undefined, body: string): Promise<void> {
  ...
  if (!signal) {
    await sendWorkspaceHatMessage(env, target, LEAD_SIGNAL_USAGE);
    return;
  }
  ...
  if (!classification.genuine) {
    ...
    await sendWorkspaceHatMessage(env, target, `Screened out, not recorded as a Lead: ${classification.reason}`);
    return;
  }
