...
test("1. Stream target helpers resolve correct Workspace (604) and System Operations (588) targets", () => {
  const env = fakeEnv();
  const ws = getWorkspaceTarget(env);
  assert.ok(ws !== null);
  assert.strictEqual(ws.chatId, -1004435157576);
  assert.strictEqual(ws.threadId, 604);
