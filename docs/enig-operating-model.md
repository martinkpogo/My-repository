...
2. Build the first new Unit (Business Development, or whichever is decided first per the "Open questions" below) entirely on the manifest/registry pattern. Zero regression risk -- there is no existing behavior to break, since the Unit doesn't exist yet. This is where the shape gets battle-tested against a real, non-toy action list, at least one real special case, and -- if the Unit needs multi-turn continuation at all -- a real `awaiting`-state handler map.
3. Once proven in production on a real Unit, migrate Sales, Marketing, Strategy, and Finance into the manifest shape one at a time, each its own shippable step with its existing test suite (`salesExecutive.test.ts`, `strategyAnalyst.test.ts`, `valueBasedPricingAssessor.test.ts`, Marketing's and `leadDiscovery.test.ts`) re-run against the refactored dispatch path to confirm behavior is unchanged, plus a manual smoke pass through each Unit's real entry flow since this touches the routing layer and continuation-state handling everything funnels through. Each Unit's special cases and `awaiting` states (the ones listed above) get explicit test coverage in their new manifest form before that Unit is considered migrated. Expect this step -- specifically porting `handleTextReply`'s existing switch cases into per-Unit `awaiting` maps -- to be the riskiest and most careful part of the whole migration, since it is the largest hand-written surface of the three and the one carrying the most existing, live conversational state.
